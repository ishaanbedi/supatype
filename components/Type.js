...
function startTypeSignedIn() {
    ...
    return (
      <>
        ...
        <span className="underline underline-offset-4 font-bold">
          {document.querySelector(".timeSpan").innerHTML} seconds
        </span>{" "}
        to complete the test!
        <br />
        Your speed for the test was{" "}
        <span className="underline underline-offset-4 font-bold">
          {document.querySelector(".scoreSpan").innerHTML}
        </span>
        .
      </>
    );
  ...
  var ZeroMsg = () => {
    return <>You did not completed the test successfully! Please try again!</>;
  };
  async function updateProfile() {
    ...
    try {
      const user = supabase.auth.user();
      scoresArrayLocal.push(
        parseInt(document.querySelector(".scoreSpan").innerHTML)
      );
  ...
  var processInput = (value) => {
    ...
    if (
      value.endsWith(" ") ||
      value === wordsObj.current[wordsObj.current.length - 1]
    ) {
  ...
  return (
    <>
      ...
      <section
        className={
          testEnded
            ? "min-h-screen md:block bg-[#1B2430] blur-sm brightness-150 "
            : "min-h-screen md:block bg-[#1B2430]"
        }
      >
        <motion.div
        ...
        >
          <h1 className="text-[#D5DFE5] text-center text-4xl font-bold py-12">
            SupaType ⚡️
          </h1>
        </motion.div>
        ...
        >
          <div className="my-4 mt-12 space-x-4 text-center">
            <button
              className="relative inline-flex items-center px-8 py-3 overflow-hidden text-[#D5DFE5] bg-[#395B64] rounded group  focus:outline-none focus:ring"
              onClick={() => {
            ...
            >
              <Link href="/">
                <span className="relative inline-flex items-center px-8 py-3 mt-2 overflow-hidden text-[#D5DFE5] border border-current rounded group  active:text-[#D5DFE5] focus:outline-none focus:ring">
                  <span className="absolute right-0 transition-transform translate-x-full group-hover:-translate-x-4">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-6 w-6"
